    ...
    switch (req.method) {
        case 'GET':
            try {
                ...
                const {nopeserta} = decodeToken(token);
                ...
                // fs.writeFileSync(path.resolve( `public/skl/${datas.nopeserta}-skl.docx`), buf);
                const bufferStream = new stream.PassThrough();

                bufferStream.end(buf);
                const filename = `${nopeserta}-skl.docx`
                res.setHeader(
                    'Content-Disposition',
